...
contract Token {
    error Token__AddressZero(address addr);
    ...
    function burn(uint256 amount) external {
        if (balances[msg.sender] < amount) revert Token__InsufficientBalance();
        balances[msg.sender] -= amount;
        totalSupply -= amount;
    ...
    function approve(address spender, uint256 value) public returns (bool) {
        if (spender == address(0)) revert Token__AddressZero(spender);
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        if (to == address(0)) revert Token__AddressZero(to);
        if (balances[msg.sender] < amount) revert Token__InsufficientBalance();
        balances[msg.sender] -= amount;
        balances[to] += amount;
    ...
    function transferFrom(address from, address to, uint256 amount) public returns (bool) {
        if (to == address(0)) revert Token__AddressZero(to);
        if (balances[from] < amount) revert Token__InsufficientBalance();
        if (allowances[from][msg.sender] < amount) revert Token__InsufficientAllowance();
